...
namespace Botble\VideoVoting\Repositories\Eloquent;
...
class VideoRepository extends RepositoriesAbstract  implements VideoInterface
{
    ...
    public function getByCategory($categoryId, $paginate = 12, $limit = 0)
    {
        ...
        $data = $this->model
            ->where('vv_videos.status', BaseStatusEnum::PUBLISHED)
            ...
            ->whereIn('vv_video_category_relation.category_id', $categoryId)
            ->select('vv_videos.*')
            ->distinct()
            ->with('slugable')
            ->orderBy('vv_videos.created_at', 'desc');
    ...
    public function getRecentPosts($limit = 5, $categoryId = 0)
    {
        ...
        if ($categoryId != 0) {
            $data = $data->join('vv_video_category_relation', 'vv_video_category_relation.video_id', '=', 'vv_videos.id')
                ->where('vv_video_category_relation.category_id', $categoryId);
        }
    ...
    public function getFilters(array $filters)
    {
        ...
        if ($filters['categories'] !== null) {
            ...
            $this->model = $this->model->whereHas('categories', function ($query) use ($categories) {
                $query->whereIn('vv_video_categories.id', $categories);
            });
        }
        ...
        if ($filters['categories_exclude'] !== null) {
            ...
            $this->model = $this->model->whereHas('categories', function ($query) use ($excludeCategories) {
                $query->whereNotIn('vv_video_categories.id', $excludeCategories);
            });
        }
